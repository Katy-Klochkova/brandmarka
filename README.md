...
- бренд-бук компании (цвета, шрифты, тон голоса, контактные данные);
- логотип компании (PNG, JPG, SVG) — опционально, отображается в визитках, презентациях, КП и email-подписях;
...
3. Готовит слоты для текста: в MVP текст берётся из JSON и шаблонов; LLM-доработка подключается отдельно через `ANTHROPIC_API_KEY`.
...
## Что сейчас использует LLM, а что — шаблоны

В MVP генерация текста **не идёт в Claude API**. Все формулировки берутся из входного JSON и фиксированных шаблонов генераторов.

| Функция | Что происходит на самом деле | LLM |
|---|---|---|
| Визитка (PNG/PDF) | Рендеринг текста из JSON через Pillow | ❌ Нет |
| Email-подпись (HTML) | Вёрстка HTML из JSON | ❌ Нет |
| Презентация (PPTX/PDF) | Слайды собираются из JSON по структуре | ❌ Нет |
| Коммерческое предложение (DOCX/PDF) | Таблица услуг и текст из JSON | ❌ Нет |
| Доработка текста под тон бренда | Подготовлено, но не реализовано | ❌ Нет |

Чтобы включить LLM, добавь `.env` с `ANTHROPIC_API_KEY` и подключи вызовы `core.llm.ask()` в генераторах презентаций, КП и подписей.
...
6. Загрузка логотипа компании (PNG, JPG, SVG) и его отображение во всех материалах.
7. Веб-интерфейс и REST API.
8. Экспорт результатов в PNG/HTML/PPTX/DOCX + PDF для визиток.
...
│   └── web/             # HTML-интерфейс
├── tests/               # минимальные тесты генераторов
...
На странице можно загрузить бренд-бук и опционально логотип, а затем сгенерировать визитку, подпись, презентацию или КП.
...
# Загрузка бренд-бука с логотипом
python run.py upload-brandbook --file data/brandbook.json --logo uploads/logo.png
```
...
### 6. Если не запускается

| Ошибка | Причина | Решение |
|---|---|---|
| `FileNotFoundError: data/brandbook.json` | Бренд-бук не найден | Загрузи через веб-интерфейс или CLI: `python run.py upload-brandbook --file data/brandbook.json` |
| `LibreOffice not found` при `--format pdf` для PPTX/DOCX | Не установлен LibreOffice | Установи LibreOffice или экспортируй только в PPTX/DOCX |
| `ModuleNotFoundError` | Не установлены зависимости | Активируй `.venv` и выполни `pip install -r requirements.txt` |
| В вебе пустая страница или не грузится UI | Сервер не поднялся / ошибка статики | Проверь, что запущен `python web.py`, и в консоли нет ошибок |

### 7. Тесты

Минимальные тесты проверяют, что все генераторы отрабатывают без ошибок и создают непустые файлы.
